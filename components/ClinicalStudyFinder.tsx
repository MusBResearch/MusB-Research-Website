...
export default function ClinicalStudyFinder() {
    ...
    const filteredStudies = studies.filter(study => {
        ...
        if (showPaidOnly && !study.isPaid) return false;
        if (showFreeTestingOnly && !study.isFreeTesting) return false;
        return true;
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-end lg:items-center gap-8 mb-12">
                        ...
                        <div className="flex items-center gap-6 shrink-0">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-slate-700 font-semibold group-hover:text-purple-600 transition-colors">Paid Studies</span>
                            </label>
                            ...
                            <label className="flex items-center gap-3 cursor-pointer group">
                                <div className="relative">
                                    <input
                                        type="checkbox"
                                        className="sr-only peer"
                                        checked={showFreeTestingOnly}
                                        onChange={() => setShowFreeTestingOnly(!showFreeTestingOnly)}
                                    />
                                    <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-cyan-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-cyan-500"></div>
                                </div>
                                <span className="text-slate-700 font-semibold group-hover:text-cyan-600 transition-colors">Free Testing</span>
                            </label>
                        </div>
                    </div>
                                ...
                                <div key={study.id} className="group relative bg-white rounded-2xl border border-slate-100 p-6 shadow-sm hover:shadow-xl hover:border-cyan-200 transition-all duration-300 flex flex-col h-full">
                                    ...
                                    <div className="space-y-3 mb-6 flex-grow">
                                        <div className="flex items-center gap-3 text-slate-600">
                                            ...
                                            <span className="font-medium text-sm">{study.compensation}</span>
                                        </div>
                                        <div className="flex items-center gap-3 text-slate-600">
